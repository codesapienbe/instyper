...
try:
    from instyper import speaker_fingerprint  # lightweight speaker signature helpers
except Exception:
    speaker_fingerprint = None  # type: Any


LOG_FILENAME = "application.log"
DEFAULT_REPO = "https://github.com/codesapienbe/instyper.git"
    ...
    class InstallerWorker(QtCore.QObject):
        ...
        def run(self):
            ...
            try:
                ...
                self.progress.emit("75:Installing instyper into the venv (uv build -> python -m pip install fallback)")
                ...
                launcher_path = self.install_dir / "instyper-launcher.py"
                ...
                if callable(make_shortcut):
                    try:
                        ...
                        make_shortcut(name="Instyper", script=str(launcher_path), icon=icon_path, desktop=True, startmenu=True)
        ...
        def _download_and_extract_zip(self, repo_url: str) -> None:
            ...
            tmpdir = tempfile.mkdtemp(prefix='instyper-download-')
            try:
...
try:
    subprocess.run([venv_python, '-m', 'uv', 'run', 'instyper'], check=True)
except Exception as e:
    print('Failed to start Instyper:', e)
    sys.exit(1)
"""
    ...
    class InstallerWindow(QtWidgets.QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("Instyper Installer")
            self.resize(700, 420)
            ...
            try:
                ...
                self.title_label = QtWidgets.QLabel('<b><span style="font-size:20pt">Welcome to Instyper</span></b>')
                self.subtitle = QtWidgets.QLabel('This installer will set up Instyper on your computer. Click Get Started to see the installation steps and options.')
            ...
            default_dir = appdirs.user_data_dir("Instyper", appauthor=False) if appdirs else os.path.expanduser("~")
        ...
        def _config_path(self) -> Path:
            ...
            try:
                if appdirs:
                    cfg_dir = Path(appdirs.user_config_dir('Instyper', appauthor=False))
                else:
                    cfg_dir = Path.home() / '.instyper'
                cfg_dir.mkdir(parents=True, exist_ok=True)
                return cfg_dir / 'installer_ui.json'
            except Exception:
                return Path.home() / '.instyper_installer_ui.json'
        ...
        def _create_installation_config(self):
            ...
            try:
                self._stages = [
                    ...
                    ("launch", "Create launcher & shortcuts", "Create a small launcher and Desktop/StartMenu shortcuts to start Instyper."),
                ]
        ...
        def _set_windows_taskbar_icon(self, hwnd: int, icon_path: str) -> None:
            ...
            try:
                ...
                try:
                    ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID('net.codesapien.instyper')
                except Exception:
                    pass
