    ...
    def run(self):
        try:
            ...
            if callable(make_shortcut):
                try:
                    make_shortcut(name="Instyper", script=str(launcher_path), icon=None, desktop=True, startmenu=True)
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="Instyper headless installer")
    p.add_argument("--repo", default=DEFAULT_REPO, help="Git repository URL to clone (default: official instyper repo)")
...
def main(argv=None):
    ...
    if args.dir:
        ...
        if appdirs:
            ...
            install_dir = Path.home() / ".instyper"
